...
- **CycloneDX** and **SPDX** SBOMs attached to the GitHub release.
- **SLSA L3 build provenance** (in-toto attestation) generated by
  `slsa-framework/slsa-github-generator`.
- **Sigstore cosign signatures** (keyless OIDC) for each wheel and
  sdist, as self-contained `.sigstore` bundles.
- Trusted Publishing to PyPI via OIDC (no static API tokens).
